            ...
            <div class="cardwrapper">                 
                ...
                <div class="card main-card" v-if="allcards[i].id != -1" @click="rotatecard">
                    ...
                    <div class="review" v-if="rotated" :class="reviewed_card[allcards[i].count]">
                        <div class="easy" @click="flag2[allcards[i].count] && review(1,allcards[i].count,$event)">EASY</div>
                        <div class="medium" @click="flag2[allcards[i].count] && review(2,allcards[i].count,$event)">MEDIUM</div>
                        <div class="hard" @click="flag2[allcards[i].count] && review(3,allcards[i].count,$event)">HARD</div>
                    </div>
                    
                </div>
                <div class="add-form card" v-if="allcards[i].id == -1">
                    <div id="cardAdd">
...
export default {
    ...
    async beforeMount() {
        if(localStorage.getItem('auth_token')){
            ...
            const resp = await fetch("http://127.0.0.1:8081/api/get/cards",{
                method:"POST",
                mode : "cors",
                headers:{
                    "Access-Control-Allow-Origin": '*',
            ...
            if(data.meta.code == 200 ){
                ...
                self.allcards.forEach(card => {
                    self.flag2[card["count"]] = true
                });
